import React, { useCallback, useEffect, useRef, useState } from "react";
  ...
  export default function AIScreen() {
    ...
    // ═══════════════════════════════════════════════════════════════════════════
    // CALL SCREEN — full screen, tab bar hidden
    // ═══════════════════════════════════════════════════════════════════════════
    if (isCallMode) {
      return (
        <View style={[s.callRoot, { backgroundColor: BG, paddingTop: topPad }]}>
          <StatusBar barStyle="light-content" backgroundColor="transparent" translucent />

          {/* Top bar — minimal: just active badge + close */}
          <View style={s.callTopBar}>
          ...
          {/* Orb area — centered, takes up most of screen */}
          <View style={s.callOrbArea}>
            {/* Glow rings — stacked behind the orb using zIndex/absolute inside a wrapper */}
            <View style={s.orbWrapper}>
              <Animated.View style={[
                s.glowRing,
                {
                  borderColor: callOrbColor + "20",
          ...
          {/* Bottom — hint + end call button */}
          <View style={[s.callBottom, { paddingBottom: Math.max(insets.bottom + 24, 40) }]}>
  ...
  const s = StyleSheet.create({
    ...
    // ── Call Screen
    callRoot: { flex: 1, flexDirection: "column" },
    callTopBar: { flexDirection: "row", alignItems: "center", justifyContent: "space-between", paddingHorizontal: 20, paddingVertical: 14 },
    ...
    // Orb wrapper: fixed size so absolute children are centered
    orbWrapper: { width: GLOW_OUTER, height: GLOW_OUTER, alignItems: "center", justifyContent: "center" },
    glowRing: {
      position: "absolute",
      width: GLOW_OUTER, height: GLOW_OUTER, borderRadius: GLOW_OUTER / 2,
      borderWidth: 1.5,
    },
    glowRingInner: {
      position: "absolute",
      width: GLOW_INNER, height: GLOW_INNER, borderRadius: GLOW_INNER / 2,
      borderWidth: 1.5,
    },
    orb: {
    ...
    callBottom: { alignItems: "center", gap: 10, paddingTop: 8 },
